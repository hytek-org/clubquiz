<x-app-layout>
    <x-slot name="header">
        <div class="py-5 -ml-4 md:pl-4 md:max-w-lg">
            <h1 class="text-2xl font-bold sm:text-5xl">Create a new category</h1>

            <p class="mt-4 text-gray-500">
                Create categories to organize content, streamline navigation, and enhance user experience
                effectively.
            </p>
        </div>
    </x-slot>
    ...
    <section class="relative flex flex-wrap lg:h-screen lg:items-center">
        <div class="w-full px-4 pb-10 sm:px-6 sm:py-16 lg:w-1/2 lg:px-8 lg:py-24">
            <form method="POST" class="mx-auto mb-0 mt-8 max-w-md space-y-4 bg-gray-50 p-4 rounded-md shadow-xl shadow-[#EDDACF]" action="{{ route('categories.store') }}"
            ...
            <div>
                ...
                <textarea id="name" name="name"
                    ...
                    rows="2" placeholder="Enter category title..." required>{{ old('name') }}</textarea>
                @error('name')
            ...
            <div class="pb-4">
                ...
                <textarea id="content" name="description"
                    ...
                    rows="4" placeholder="Enter category description..." required>{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-red-500">{{ $message }}</p>
                @enderror
            </div>
