<div>
    <x-slot name="header">
        <div class="-ms-5 md:-ms-0  py-10">
            <h2 class="text-3xl md:text-[3rem] font-medium">Leaderboard</h2>
        </div>
        <div class="flex   ">

            <div class="w-full bg-black  text-white m-1  px-4 rounded-md lg:w-[56rem] pt-20 lg:pt-4 -ms-7  md:-ms-0 ">
                <div class="flex justify-center h-full  ">

                    <div class="grid grid-cols-3 gap-8 lg:pt-32">
                        <div class="relative lg:mt-20">
                            <div class="h-32 lg:h-full w-16 lg:w-32 bg-blue-500 rounded-t-md">
                                <span
                                    class="absolute top-0 left-0 text-white text-[0.8rem] lg:text-[1.5rem] font-bold px-2 pt-2 lg:pt-4">2nd</span>
                            </div>
                            <div class="absolute inset-0 flex flex-col items-center justify-center space-y-1">
                                @if (isset($quizzes[1]->user))
                                    <img class="w-10 rounded-full" src="{{ asset('assets/avatars/male.jpg') }}"
                                        alt="avatar">
                                    <a class="cursor-pointer hover:underline text-xs"
                                        href="{{ route('profile.show', ['username' => $quizzes[1]->user->username]) }}"><span>@</span>{{ $quizzes[1]->user->username }}</a>
                                @endif
                            </div>
                        </div>
                        <div class="relative">
                            <div class="h-32 lg:h-full w-16 lg:w-32 bg-yellow-500 rounded-t-md">
                                <span
                                    class="absolute top-0 left-0 text-white text-[1rem] lg:text-[1.5rem] font-bold px-2 pt-2 lg:pt-4">1st</span>
                            </div>
                            <div class="absolute inset-0 flex flex-col items-center justify-center space-y-1">
                                @if (isset($quizzes[0]->user))
                                    <img class="w-10 rounded-full" src="{{ asset('assets/avatars/male.jpg') }}"
                                        alt="avatar">
                                    <a class="cursor-pointer hover:underline text-xs"
                                        href="{{ route('profile.show', ['username' => $quizzes[0]->user->username]) }}"><span>@</span>{{ $quizzes[0]->user->username }}</a>
                                @endif
                            </div>
                        </div>
                        <div class="relative lg:mt-20">
                            <div class="h-32 lg:h-full w-16 lg:w-32 bg-violet-500 rounded-t-md">
                                <span
                                    class="absolute top-0 left-0 text-white text-[0.8rem] lg:text-[1.5rem] font-bold px-2 pt-2 lg:pt-4">3rd</span>
                            </div>
                            <div class="absolute inset-0 flex flex-col items-center justify-center space-y-1">
                                @if (isset($quizzes[2]->user))
                                    <img class="w-10 rounded-full" src="{{ asset('assets/avatars/male.jpg') }}"
                                        alt="avatar">
                                    <a class="cursor-pointer hover:underline text-xs"
                                        href="{{ route('profile.show', ['username' => $quizzes[2]->user->username]) }}"><span>@</span>{{ $quizzes[2]->user->username }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="hidden lg:block bg-green-500 w-[44rem] m-1 rounded-md">
                <section class="flex flex-col justify-center antialiased bg-gray-50 text-gray-600  p-4">
                    <div class="h-full">
                        <!-- Card -->
                        <div class="max-w-xs mx-auto">
                            <div class="relative bg-indigo-700  rounded-lg shadow-lg p-5 overflow-hidden">
                                <!-- Illustration -->
                                <svg class="absolute top-0 right-0" width="158" height="119" aria-hidden="true">
                                    <defs>
                                        <linearGradient x1="50%" y1="0%" x2="50%" y2="96.22%"
                                            id="a">
                                ...
                                <div class="relative pt-[6.25rem] pb-14">
                                    <div class="text-xs font-bold uppercase text-green-400 tracking-widest mb-2">
                                        ClubQuiz Explore
                                    </div>
                                    ...
                                    <p class="text-indigo-200">
                                        Explore a wide range of quizzes created by users around the world. Find quizzes
                                        on various topics and challenge yourself!
                                    </p>
                                </div>
                                ...
                                <div class="relative text-right">
                                    <a class="inline-flex w-11 h-11 justify-center items-center bg-green-400 hover:bg-green-300 text-pink-50 hover:text-white rounded-full transition duration-150"
                                        href="{{ route('explore') }}"><span class="sr-only">Read more</span> <span
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="overflow-x-auto -ms-4 md:-ms-0 mt-4">
                        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm rounded-md border">
                            <thead class="text-left">
                                <tr>
                                    <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Userame</th>
                                    <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Total time played</th>
                                    <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Accuracy</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($users as $user)
                                    <tr @class([
                                        'bg-slate-100' =>
                                            auth()->check() && $user->username == auth()->user()->username,
                                    ])>
                                        <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-900"><a
                                                href="{{ route('profile.show', ['username' => $user->username]) }}"
                                                class="underline hover:text-blue-500"><span>@</span>{{ $user->username }}</a>
                                        </td>
                                        <td class="whitespace-nowrap px-4 py-2 text-gray-700 ">
                                            {{ \Carbon\CarbonInterval::seconds($user->time_spent)->cascade()->forHumans([ 'parts' => 1]) }}


                                        </td>
                                        <td class="whitespace-nowrap px-4 py-2">
                                            <p
                                                class="inline-block rounded-full bg-indigo-600 p-2.5 text-xs font-medium text-white hover:bg-indigo-700">
                                                {{ min(round(($user->correct / $total_questions) * 100, 2), 100) }}%
                                            </p>
                                        </td>
                                    </tr>
                                @empty
